...
export default class PopulateData extends Component
{
    ...
    componentDidMount()
     {
        ...
        axios.get("./shopstock.json").then((resp)=>{
            ...
            let arr=resp.data
           // console.log(arr)
            let temp = arr.map((e,index)=>{return <option key={index} value={e.id}>{e.id}</option>})
        ...
        axios.get("./shopstock.json").then((resp)=>{
            ...
            let arr=resp.data
            console.log(arr)
             let temp = arr.map((e,index)=>{return <p> key={index} value={e.id}{e.id}</p>})
             this.setState({optionarr:temp})
    ...
    render()
    {
        return(
        <div>
            <select onChange={this.handler1}>{this.state.optionarr}</select>
            <p>{}</p>
        </div>)
    }
}